...
export interface CustomizeGSSPParams<Q extends ParsedUrlQuery = ParsedUrlQuery, D extends PreviewData = PreviewData> {
  scope: Scope;
...
> = (params: CustomizeGSSPParams<Q, D>) => GetServerSidePropsResult<P> | Promise<GetServerSidePropsResult<P>>;

export interface CreateGSSPConfig<P extends AnyProps, Q extends ParsedUrlQuery, D extends PreviewData> {
  pageEvent?: EmptyOrPageEvent<any, any>;
  customize?: CustomizeGSSP<P, Q, D>;
}

export function createGSSPFactory({ sharedEvents = [], createServerScope = () => fork() }: CreateAppGSSPConfig = {}) {
  return function createGSSP<
    P extends AnyProps = AnyProps,
    Q extends ParsedUrlQuery = ParsedUrlQuery,
    D extends PreviewData = PreviewData
  >({ pageEvent, customize }: CreateGSSPConfig<P, Q, D> = {}): GetServerSideProps<P, Q, D> {
    return async function getServerSideProps(context) {
       ...
       */
      const gsspResult = customize ? await customize({ scope, context }) : { props: {} as P };
